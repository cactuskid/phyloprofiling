...
def get_replacement_dict(h5file, oma_id_obj):

    replacement_dic = {}
    genome_list = []

    for genome in h5file.root.Genome.read():
        genome_list.append(genome[5].decode())

    uniprot_species = re.findall(r'\b[A-Z]{5}\b', ' '.join(genome_list)) + ["STAA3", "ECO57", "BUCAI", "CHLPN"]
    ...
    for genome in genome_list:
        if any(character in genome for character in char_to_remove):
            replacement_dic[genome] = replace_characters(genome, char_to_remove)
...
def replace_characters(string, char_to_remove):
    """
    Replace character from string
    :param string: string to correct
    :param char_to_remove: list of characters to remove
