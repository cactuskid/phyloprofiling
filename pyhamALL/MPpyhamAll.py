...
if __name__ == '__main__':
    ...
    def matrixupdater(i, matq , retq , l , rows):
        with h5sparse.File(config.datadirLaurent + datestr + "matrix.h5",'w'  ) as h5matrix:

            hogmat = csr_matrix( (rows , 4*len(taxaIndex)  ) )

            sparsemat = scipy.sparseCSR()
            while True:
    ...
    def saver( i,q,retq,l ):
        ...
        with open('errors.txt' , 'w')as errfile:
            ...
            with  h5py.File(config.datadirLaurent+ datestr + 'hashes.h5','w', libver = 'latest') as h5hashes:
                dsets = {}
                ...
                while True:
                    ...
                    if thisdf is not None:
                        ...
                        if time.clock() -savestart > 2000:
                            print(thisdf['Fam'].max())
                            print('saving')
                            with open(config.datadirLaurent + datestr + '_'+str(threshold)+'_' +'newlsh.pkl' , 'wb') as lshout:
                                    with open(config.datadirLaurent +datestr+ 'newlshforest.pkl' , 'wb') as forestout:
