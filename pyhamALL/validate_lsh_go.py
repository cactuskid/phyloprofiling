...
with  h5py.File(config.datadir+ 'hashes.h5', 'r') as h5hashes:
	...
	for n in range(1,len(events)):
		for events_combo in itertools.combinations(events, n+1):
			for fam_query in fam_list_queries:
				
				# get hash for this hog
				hash_query = profileGen.get_hash_hog_id(fam_query, events_combo)	
				# get the results for this query in the lsh
				lsh_results_unfiltered = lsh.query(hash_query)
				# add the query to the list of results and filter the results (if no go term, remove hog id)
				lsh_results_filtered = [fam_query] +_[result in lsh_results_unfiltered if goTermAnalysis.get_go_terms(result_fam_id(result))]

				# get a list of hashes, one for each filtered results
				hashes = [ hog_id, profileGen.get_hash_hog_id(hog_id, events_combo) for hog_id in lsh_results_filtered ]				
				...
				# save matrices
				matrices_dict[(fam_query, events_combo)] = {'semantic_distance':semantic_distance, 'jaccard_distance':jaccard_distance}
